...
```
非推奨警告:
yt-dlpの最新バージョンでは、ffmpegを使用して複数のフォーマットを同時に標準出力にストリームできます。
そのため、将来的なバージョンでは、これに対するデフォルトが `-f bv*+ba/b` に設定される予定です。
...
```
非推奨警告:
以下で説明する動作は複雑で直感的でないため、将来的にはこれが削除され、マルチストリームがデフォルトで有効になります。
